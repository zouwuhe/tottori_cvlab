...
#include <iostream>
#include <fstream>
//note: 由于kinect具有特殊的投影方式，因此处理kinect图像的基本思想是：获取图像点坐标的时候，都是先将kinect图像转化成普通中心投影图像(横向翻转)，然后获取像点坐标。
//而输出图像点坐标的时候，都是先将图像点坐标再次左右翻转，转化到kinect图像下的坐标，然后在kinect图像上输出像点(画出坐标对应的像点)。
using namespace std;

int test_num=0;
const int board_dt=80;//wait 80 frames per chessboard view
...
int main(){
	char* filename="experiment_test8_8_3.txt";
	...
	//////////////////////////////////////////*///实验证明单位化对校正没有帮助，反而有不好的影响，容易导致结果发散，可能是单位化的过程引入了误差->其实不是单位化的问题，而是lambda的调节可以控制收敛时间
		...
		if(frame++%board_dt==0){//每间隔board_dt帧尝试一次 采集图像上的棋盘角点
			...
			if(corner_count==board_n){
				for(int i=0;i<board_n;i++){
					...
					CV_MAT_ELEM(*image_points1,float,i,1)=image->height-corners[i].y;// according to the feature of kinect, y should be overturned. (Here, y is the direction of width,meaning y is horizontal in image)
					...
					CV_MAT_ELEM(*object_points2,float,i,1)=(float)(i%board_w);//注意，此处将baord_w定为y
					CV_MAT_ELEM(*object_points2,float,i,2)=1.0f;
				}
				cvFindHomography(object_points2,image_points2,Hd);
				//draw P in the chessboard of kinect image
				...
				//计算image_points以及单映性矩阵Hd,标定板上坐标Po等所有操作都是先将kinect拍到的图像转换到普通的中心投影图像以后进行的，因此所有的结果都是对应普通中心投影图像。
				//所以计算出的laser_source也是在普通中心投影图像下的坐标，如果要在kinect图像上显示就必须先转化回到kinect图像上的像点坐标
				...
				for(int y=0;y<difference_image->height;y++){
					...
					for(int x=0;x<difference_image->width;x++){
						ptr_difference[x]=sqrt(pow(ptr_colorframe2[3*x]-ptr_original[3*x],2.0)+pow(ptr_colorframe2[3*x+1]-ptr_original[3*x+1],2.0)+pow(ptr_colorframe2[3*x+2]-ptr_original[3*x+2],2.0));
				...
				cvCircle(colorframe2,laserpoint_q,2,cvScalar(0,0,255),2,8,0);
				//////////////////////////////////////////////////////////////////////////////////////////////////////////////
