//针对基于两个kinect的组合，实现驾驶员的视野区域的自动提取的系统，该函数主要实现该系统的参数标定。
//主函数中：计划通过人在rgb图像上选取自己正在看的视点，实时读取系列观测数据，并存在矩阵中。
//本函数中：再利用这些观测数据，在本函数中实现一系列参数的标定，即kinect1与kinect2 的相对姿势、人的人脸方向与视线方向的偏差校正矩阵。
//observed quantity:r1,r2,r3,Px,Py,Pz,Qx,Qy,Qz; 
//parameter:theta,phi,omega,alpha,beta,X0,Y0,Z0 corresponding to x[0],x[1]... ...x[7];
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
#include <iostream>
...
void calibration(char *filename,double fx,double fy,double cx,double cy,int *qa,int *qb,double *n1,double *n2,double *n3,double *Tpx,double *Tpy,double *Tpz,double *x,int m){
	...
	double *r2=(double *)malloc(m*sizeof(double));
	double *r3=(double *)malloc(m*sizeof(double));
	double *Px=(double *)malloc(m*sizeof(double));
	double *Py=(double *)malloc(m*sizeof(double));
	double *Pz=(double *)malloc(m*sizeof(double));
	double *Qx=(double *)malloc(m*sizeof(double));
	double *Qy=(double *)malloc(m*sizeof(double));
	double *Qz=(double *)malloc(m*sizeof(double));
	///r1,r2,r3,Px,Py,Pz,Qx,Qy,Qz 为已知量  参数x[0]-x[7] 已经赋值  准备矩阵 fx Jx
	//其中的m,r1,r2,r3,Px,Py,Pz,Qx,Qy,Qz 也可以作为参数传入本程序，并在此处完成给这些观测数据赋值。*/
